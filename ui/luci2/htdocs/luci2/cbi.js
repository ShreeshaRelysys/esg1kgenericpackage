(function() {
	...
	validation.types = {
        ...
        'rt_alphanumeric': function()
		{
			if ((this.length <= 253) &&
			    ((this.match(/^[a-zA-Z0-9_/]+$/) != null ||
			     (this.match(/^[a-zA-Z0-9_][a-zA-Z0-9_/]*[a-zA-Z0-9]$/) &&
			      this.match(/[^0-9.]/)))))
				return true;

			validation.i18n('Must be a valid alphanumeric');
			return false;
		},
		...
		'rt_float': function()          
        {
                ...
                validation.i18n('Must be a valid float number');
        		return false;           
		},
		...
		{
			if 
			    ((this.match(/^-?((1?[0-7]?|[0-9]?)[0-9]|179)\.[0-9]{1,6}?([n]|[N]?|[e]|[E]?|[w]|[W]?|[s]|[S])$/) != null ))
                                        return true;

			validation.i18n('Must be valid gps data');
			return false;
		},
                
        'rt_mqtttopic': function()
		{
			if ((this.length <= 253) &&
			    ((this.match(/^[a-zA-Z0-9_/]+$/) != null ||
			     (this.match(/^[a-zA-Z0-9_][a-zA-Z0-9_/]*[a-zA-Z0-9]$/) &&
			...
			validation.i18n('Must be a valid data');
			return false;
		},

		'rt_phonedigit': function()
		{
			if (this.match(/^[0-9\*#!\.]+$/) != null)
				return true;
        ...
        'rt_hexadecimal': function()
		{
			if (this.match(/^[A-Fa-f0-9]+$/) != null)
				return true;

			validation.i18n('Must be a valid hexadecimal');
			return false;
		}
	};
	...
	cbi_class.AbstractValue = L.ui.AbstractWidget.extend({
		...
		handleValidate: function(ev)
		{
			...
			return rv;
                ...
                handleValidateDate: function(ev)
                {
                        var $elem = $(this);
                        var dateElID=$elem.attr('id');
                        $('#'+dateElID).combodate();
                },
                
                handleValidatetime:function(ev)
                {
                        var $elem = $(this);
                        var timeElID=$elem.attr('id');
                        $('#'+timeElID).combodate();
                },
		...
		attachEvents: function(sid, elem)
		{
            ...
            if (elem.hasClass('date-class'))
            {
            	elem.click(evdata, this.handleValidateDate);
            	elem.change(evdata, this.handleValidate);
			} 
                        
            if (elem.hasClass('time-class'))
            {
            	elem.click(evdata, this.handleValidatetime);
                elem.change(evdata, this.handleValidate);
			}
	...
	cbi_class.CheckboxValue = cbi_class.AbstractValue.extend({
		widget: function(sid)
		{
			...
			if (typeof(o.readonly) == 'undefined') o.readonly = '0';
	...
	cbi_class.ReadonlyInputValue = cbi_class.AbstractValue.extend({                                                   
                widget: function(sid)                                                                                     
                {                                                                                                         
                        var i = $('<input />')                                                                            
                                .addClass('form-control')                                                                 
	...
	cbi_class.DateValue = cbi_class.AbstractValue.extend({
    	widget: function(sid)
		{
        	var i = $('<input />')
	...
	cbi_class.TimeValue = cbi_class.AbstractValue.extend({
    	widget: function(sid)
		{
         	var i = $('<input />')
                .addClass('form-control time-class')
				.attr('id', this.id(sid))
				.attr('data-custom-class', 'form-control')
				.attr('type', 'text')
				.attr('data-format', 'HH:mm:ss')
				.attr('data-template', 'HH:mm:ss')
				.attr('type', 'text')
				.val(this.ucivalue(sid));
	...
	cbi_class.ComboBox = cbi_class.AbstractValue.extend({
		_change: function(ev)
		{
			...
			if(self.options.listcustom==true|| self.options.listcustom== undefined ){
			...
			else if(self.options.listcustom==false)
			{
				if (self.options.optional && s.selectedIndex == 0)
				{
					ev.data.input.val('');
				}
				else
	...
	cbi_class.DynamicList = cbi_class.ComboBox.extend({
		...
		_btnclick: function(ev)
		{
			if (!this.getAttribute('disabled'))
			{
				...
				{
					if(ev.data.listlimit && ev.data.index >= (ev.data.listlimit-1)){
						return false;
					}
					ev.data.self._redraw(NaN, ev.data.index, -1, ev.data);
				}
                ...
                commit: function(parent_sid)
		{
			...
			for (i = 0; i < s.length; i++)
			{
				if (!this.options.readonly)
				{
					for (var f in this.fields)
					{
						if (typeof(this.fields[f].commit) != 'function')
							continue;

						var rv = this.fields[f].commit(s[i]['.name']);
						if (L.isDeferred(rv))
							deferreds.push(rv);
					}
				}
				...
				for (var j = 0; j < this.subsections.length; j++)
				{
					var rv = this.subsections[j].commit(s[i]['.name']);
					deferreds.push.apply(deferreds, rv);
				}
			}
		...
		renderFooter: function()
		{
			...
			return $('<div />')
				...
				.append($('<div />')
					...
					.append(L.ui.button(L.tr('Save'), 'primary')
